...
async def test_client_query_data_as(mocker):
    client = GraphQLClient(endpoint="http://test", codec=DefaultGraphQLCodec(), validate=False)
    ...
    mock_resp_ctx.__aenter__.return_value.read = mocker.AsyncMock(return_value=b'{"data": {"cities": [{"name": "London"}, {"name": "Paris"}]}}')
...
async def test_client_encode_variables(mocker):
    ...
    mock_resp_ctx = mocker.AsyncMock()
    mock_resp_ctx.__aenter__.return_value = mocker.Mock()
    mock_resp_ctx.__aenter__.return_value.read = mocker.AsyncMock(return_value=b'{"data": {"createCity": {"name": "Berlin"}}}')
    mock_resp_ctx.__aenter__.return_value.status = 200
    
    mock_request = mocker.patch("aiohttp.ClientSession.request", return_value=mock_resp_ctx)
    
    await client.query("mutation($input: CityInput) { createCity(input: $input) { name } }", variables=variables)
